...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(screenWidth, screenHeight, "Phong Shading", NULL, NULL);
        ...
        for (int i = 0; i < screenWidth; i++){
            ...
            if (closest != 0){
                if (closest == 1){
                    ...
                    if (hit_shad == true){
                        Image[pixel_index] = pow(l1a.x(), 1/gamma) * 255;
                        Image[pixel_index + 1] = pow(l1a.y(), 1/gamma) * 255;
                        Image[pixel_index + 2] = pow(l1a.z(), 1/gamma) * 255;
                    }
                    else{
                        std::cout << 5 << std::endl;
                        Image[pixel_index] = (pow(l1.x() > 1 ? 1: l1.x(), 1/gamma) * 255);
                        Image[pixel_index + 1] = (pow(l1.y() > 1 ? 1: l1.y(), 1/gamma) * 255);
                        Image[pixel_index + 2] = (pow(l1.z() > 1 ? 1: l1.z(), 1/gamma) * 255);
                    }
                }
                else if (closest == 2){
                    vec3 v = (-r.direction()) / (-r.direction()).length();
                    ...
                    if (hit_shad == true){
                        Image[pixel_index] = pow(l2a.x(), 1/gamma) * 255;
                        Image[pixel_index + 1] = pow(l2a.y(), 1/gamma) * 255;
                        Image[pixel_index + 2] = pow(l2a.z(), 1/gamma) * 255;
                    }
                    else{
                        Image[pixel_index] = (pow(l2.x() > 1 ? 1: l2.x(),1/gamma) * 255);
                        Image[pixel_index + 1] = (pow(l2.y() > 1 ? 1: l2.y(),1/gamma) * 255);
                        Image[pixel_index + 2] = (pow(l2.z() > 1 ? 1: l2.z(),1/gamma) * 255);
                    }
                }
                else if (closest == 3){
                    vec3 v = (-r.direction()) / (-r.direction()).length();
                    ...
                    if (hit_shad == true){
                        Image[pixel_index] = pow(l3a.x(), 1/gamma) * 255;
                        Image[pixel_index + 1] = pow(l3a.y(), 1/gamma) * 255;
                        Image[pixel_index + 2] = pow(l3a.z(), 1/gamma) * 255;
                    }
                    else{
                        Image[pixel_index] = (pow(l3.x() > 1 ? 1: l3.x(), 1/gamma) * 255);
                        Image[pixel_index + 1] = (pow(l3.y() > 1 ? 1 : l3.y(), 1/gamma) * 255);
                        Image[pixel_index + 2] = (pow(l3.z() > 1 ? 1: l3.z(), 1/gamma) * 255);
                    }
                }
                else if (closest == 4){
                    vec3 v = (-r.direction()) / (-r.direction()).length();
                    ...
                    if (hit_shad == true){
                        Image[pixel_index] = pow(l_pa.x(), 1/gamma) * 255;
                        Image[pixel_index + 1] = pow(l_pa.y(), 1/gamma) * 255;
                        Image[pixel_index + 2] = pow(l_pa.z(), 1/gamma) * 255;
                    }
                    else{
                        Image[pixel_index] = (pow(l_p.x() > 1 ? 1: l_p.x(), 1/gamma) * 255);
                        Image[pixel_index + 1] = (pow(l_p.y() > 1 ? 1: l_p.y(), 1/gamma) * 255);
                        Image[pixel_index + 2] = (pow(l_p.z() > 1 ? 1: l_p.z(), 1/gamma) * 255);
                    }
                }
            }
            else{
